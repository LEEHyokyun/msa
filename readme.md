...
## 16. Caching - 게시글 목록(페이징) 조회 최적화

> 게시글 페이징 및 무한 스크롤 목록 조회 최적화를 통한 목록 조회 기능을 향상한다.
- 기존 캐싱 전략(Cachable)을 페이징 목록 조회에 그대로 적용할 경우 원본데이터 동기화 불가능으로 인한 캐싱불가(과거데이터 추출) 문제가 발생한다.
- 최신글 1000개 위주로 캐싱(hot data)하며, 1000개 이후의 데이터는 원본 데이터로 요청하여 받아온다(cold data).
- 즉, 기존 이벤트를 통해 비동기로 모든 게시글/댓글/좋아요 데이터를 redis에 저장한 방식에서 필요 데이터에 대해서만 캐싱하는 전략으로 변경한다.

## 17. Caching Concurrency - 동시 요청에 대한 캐싱 중복 방지

> 동시요청의 상황에서 캐싱 중복 및 캐싱만료 시 원본데이터 중복 요청 등의 불필요한 소모를 제거한다.
- 기존 단순 Cachable 사용으로 인한 동시성 제어 불가 제약으로, 캐싱을 동시성 상황에서 효율적인 동작이 가능하도록 전략을 수립한다.
- logical TTL과 physical TTL을 분리하여 운용하며, physical TTL 시간을 더 길게 설정한다.
- 동시에 여러 요청이 발생하여도, 일부 요청에 대해선 일관성을 어느 정도 포기하여 즉시 응답(캐싱)이 올 수 있도록 동작하도록 한다.

[Request Collapsing]
- 분산락을 도입하여 동시요청 시 캐싱적재 책임을 하나의 요청에만 위임하며, 캐싱 동시 적재가 발생하지 않도록 한다.
- 모든 동시 요청에 대해 원본 데이터 요청 및 캐싱 적재, 이외는 과거 데이터 캐싱 등으로 중복으로 발생할 수 있는 각 요청을 전략에 따라 붕괴시킨다(Request Collapsing).
